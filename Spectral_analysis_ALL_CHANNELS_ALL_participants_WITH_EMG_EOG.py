...
for eeg_file in eeg_files:
    ...
    #         sls = yasa.SleepStaging(raw, eeg_name=channel, eog_name="EOG2",emg_name="EMG")
    ...
    #         # Append accuracy information to the DataFrame
    #         accuracy_data = pd.concat([accuracy_data, pd.DataFrame({
    #             'Participant': [eeg_file],
    #             'Channel': [channel],
    #             'Accuracy': [accuracy_channel]
    #         })], ignore_index=True)
    ...
    for channel in chan:
        try:
            ...
            # Calculate accuracy for the current channel
            accuracy_channel = accuracy_score(hypno, hypno_pred_channel)
...
accuracy_data.to_csv("C:/Users/danie/OneDrive/Desktop/Biomedical_engineering_PHD/Argenina_pasantia/EEG_DATA/accuracy_result_with_EOG2_EMG.csv", index=False, float_format='%.0f%%')
